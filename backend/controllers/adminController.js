...
export const updatePersonalDetails = async (req, res) => {
  try {
    ...
    Object.entries(changed_data).forEach(([key, value]) => {
      ...
      if (!val) errors[key] = `${key} cannot be empty`;
      else if (val.length < 3) errors[key] = `${key} must be at least 3 characters`;
      else if (val.length > 50) errors[key] = `${key} must be less than 50 characters`;
      if (key === "email" && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) errors[key] = "Invalid email format";
    });
    if (Object.keys(errors).length) return res.status(400).json({ errors });

    const updatedAdmin = await Admin.findByIdAndUpdate(ADMIN_ID, { $set: changed_data }, { new: true });
    if (!updatedAdmin) return res.status(404).json({ error: "Admin not found." });

    res.status(200).json({ message: "Personal details updated successfully", admin: updatedAdmin });
  } catch (err) {
    console.error("Backend error:", err);
